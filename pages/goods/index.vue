<template>
	<view class="u-wrap">
		<view class="u-search-box">
			<u-search :clearabled="true" :show-action="false" @search="goodsSearch" v-model="searchVal"></u-search>
		</view>
		<view class="u-menu-wrap">
			<scroll-view scroll-y scroll-with-animation class="u-tab-view menu-scroll-view" :scroll-top="scrollTop">
				<block v-for="(item,index) in tabbar" :key="item.id">
					<view class="nav-title u-flex">{{item.name}}</view>
					<view v-for="(i,ci) in item.children" :key="i.id" class="u-tab-item u-flex-col"
						:class="[current==`${index}-${i.id}` ? 'u-tab-item-active' : '']"
						:data-current="`${index}-${i.id}`" :data-name="i.name"
						@tap.stop="swichMenu(`${index}-${i.id}`,$event)">
...
<script>
	export default {
		...
		methods: {
			async goodsSearch() {
				const res = await this.$u.api.goodsSearch({
					title: this.searchVal
				});
				this.goodsTitle = "搜索结果";
